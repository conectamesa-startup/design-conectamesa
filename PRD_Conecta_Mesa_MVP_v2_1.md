...
| [🤝 ONG/B2G] | ONG / Cozinha Solidária | Entidades sociais que recebem doações; isentas de comissão |
...
| F07 | Validação de validade por OCR: leitura da data de validade via câmera, bloqueando produto vencido | [🏪 Comerciante B2B] [👤 PF Doador] | google_mlkit_text_recognition |
...
| Sistema de avaliações pós-retirada | Previsto no médio prazo |
...
4. Toca em "Anunciar" → captura foto via câmera → OCR valida a data de validade
...
4. Reserva itens do pool de doações (take rate = 0%)
...
| US-04 | [🏪 Comerciante B2B] [🤝 ONG/B2G] | Como empresa ou ONG, quero escolher entre cadastro PF e PJ. | Cards comparativos PF vs PJ. Formulário PJ com CNPJ, razão social e segmento. Enum user_roles salvo separadamente. | 🔴 Alta |

### Módulo: Marketplace

| ID | Tipo | História | Critérios de Aceitação | Prioridade |
|----|------|----------|------------------------|------------|
| US-05 | [🛒 Consumidor B2C] [🤝 ONG/B2G] | Como consumidor ou ONG, quero ver feed ordenado por proximidade GPS. | Ordenação por distância via Haversine. Permissão de localização solicitada. Fallback para cidade do cadastro se GPS negado. | 🔴 Alta |
...
| US-08 | [🏪 Comerciante B2B] [👤 PF Doador] | Como vendedor, quero publicar anúncio via câmera com GPS automático. | Câmera nativa abre via image_picker. GPS em até 5s. Formulário ≤ 3 telas. Publicação em < 60s. | 🔴 Alta |
| US-09 | [🏪 Comerciante B2B] [👤 PF Doador] | Como vendedor, quero que produto vencido seja bloqueado automaticamente. | OCR lê data em tempo real. Bloqueio imediato se data pretérita. Log de tentativa salvo. | 🔴 Alta |
...
| RF-01.5 | [⚙️ Admin] | Tabela user_roles separada de profiles com enum (admin \| pf \| empresa \| ong) | PostgreSQL + RLS | ✅ MVP |
...
| ID | Tipo | Requisito | Impl. Técnica | Status |
|----|------|-----------|---------------|--------|
| RF-02.1 | [🛒 Consumidor B2C] [🤝 ONG/B2G] | Feed com busca textual, filtros por categoria/tipo e ordenação por GPS | Supabase Database + geolocator | ✅ MVP |
| RF-02.2 | [🛒 Consumidor B2C] [🤝 ONG/B2G] | Card de anúncio: foto, nome, desconto, distância, tempo restante e selo | Flutter Widget | ✅ MVP |
| RF-02.3 | [🛒 Consumidor B2C] [🤝 ONG/B2G] | Tela de detalhe: foto HD, descrição, macronutrientes, alérgenos, validade, GPS, chips e CTA sticky | Flutter Screen | ✅ MVP |
...
| RF-02.5 | [🏪 Comerciante B2B] [👤 PF Doador] | OCR on-device para validar data de validade; bloqueia publicação se vencido | google_mlkit_text_recognition | ✅ MVP |
...
| RF-03.8 | [🤝 ONG/B2G] | Transações B2G (ONGs verificadas) têm take rate = 0% — sem split, produto transferido diretamente sem comissão | Asaas — sem split configurado | ✅ MVP |
...
| RF-04.3 | [🏪 Comerciante B2B] [👤 PF Doador] | Saque via Pix: transferência do saldo DISPONIVEL para chave Pix cadastrada; saque só permitido após status = DISPONIVEL | Asaas POST /v3/transfers | ✅ MVP |
...
| RF-04.7 | Sistema | Disputa com status DISPONIVEL abre ticket interno com prazo de 48h para análise pela equipe Conecta Mesa | Supabase Database + fluxo interno | ✅ MVP |

### RF-05: Chat e Notificações

| ID | Tipo | Requisito | Impl. Técnica | Status |
|----|------|-----------|---------------|--------|
| RF-05.1 | [Todos] | Chat assíncrono por anúncio com histórico persistido e mensagens em tempo real | Supabase Realtime + tabela messages | ✅ MVP |
| RF-05.2 | [Todos] | Push notifications para: nova mensagem, confirmação de pagamento, alerta de produto próximo, produto convertido para doação, saldo liberado na carteira | flutter_local_notifications + Supabase | ✅ MVP |

### RF-06: Dashboard de Impacto

| ID | Tipo | Requisito | Impl. Técnica | Status |
|----|------|-----------|---------------|--------|
| RF-06.1 | [Todos] | 4 cards de KPI: CO₂ evitado (2,5 kg/kg — FAO 2013), kg resgatados, refeições (1,2/kg — WRAP UK), valor salvo (R$ 9,50/kg) | constants.dart | ✅ MVP |
...
| RF-07.2 | [🏪 Comerciante B2B] [🤝 ONG/B2G] | Formulário de verificação com upload de CNPJ + documentos; validação via Brasil API; aprovação manual admin | Supabase Storage + Brasil API | ✅ MVP |
...
|-----------|------|-------------|
| Feed geolocalizado | < 2s em 4G | Flutter DevTools |
...
| OCR de validade on-device | < 2s | Flutter profiler |
...
- 3-D Secure 2.0 obrigatório acima de R$ 200; Asaas Score em todas as transações
...
- Tipografia: Montserrat (500–800) para títulos; Nunito (400–700) para corpo via google_fonts
...
|----|------|-------|-----------------|
| RN-01 | [🏪 Comerciante B2B] [👤 PF Doador] | Produto com data de validade vencida (OCR on-device) NÃO pode ser publicado | Bloqueio imediato + alerta + log de tentativa |
...
| RN-03 | [🤝 ONG/B2G] | ONGs com Selo ONG Verificada acessam pool de doações sem comissionamento (take rate = 0%) | Filtro de feed por tipo = 'doacao'; campo comissao = 0 na transação B2G |
...
| PAGO | [🛒 Consumidor B2C] [🏪 Comerciante B2B] | Pagamento confirmado; saldo marcado como PENDENTE na carteira | → ENTREGUE (QR Code) → CANCELADO (não retirado em 2h) |
| ENTREGUE | [🛒 Consumidor B2C] [🏪 Comerciante B2B] | QR Code confirmado; saldo muda para DISPONIVEL na carteira | Estado final (sucesso) |
| CANCELADO | [🛒 Consumidor B2C] [🏪 Comerciante B2B] | Pedido cancelado; estorno automático iniciado | → Asaas Refund API automático |
...
| RN-07 | [🤝 ONG/B2G] | Transações B2G têm take rate = 0% — isentas por design estratégico |
| RN-08 | [🛒 Consumidor B2C] | Reserva PIX expira em 15 minutos sem pagamento; produto retorna ao status DISPONIVEL |
| RN-09 | [🛒 Consumidor B2C] | Produto pago e não retirado em 2h: pedido cancelado e estorno automático via Asaas Refund API |
...
| RN-12 | Sistema | Estorno em pedido PENDENTE é automático via API; estorno em pedido DISPONIVEL requer análise interna em até 48h |
...
| 11 | Dashboard de Impacto atualizado | Supabase Database |
...
| 3b | NÃO (status = DISPONIVEL) → abre ticket interno com prazo de 48h para análise da equipe Conecta Mesa | Fluxo interno |
...
| 🗺️ | **Mapa** | Mapa interativo com pins dos anúncios |
| ➕ | **Anunciar** | Se não verificado → redireciona para verificação. Se verificado → formulário de anúncio |
| 👛 | **Carteira** | Se não verificado → redireciona para verificação. Se verificado → saldo disponível, saldo pendente, histórico de vendas e saque via Pix |
| 📋 | **Histórico** | Cards de compras com status: *A pagar / A retirar / Finalizado*. Se em aberto → chat com vendedor |
...
|-------|-----|---------------|
| 🏠 | **Início** | Feed de anúncios de vendas e doações |
| 🗺️ | **Mapa** | Mapa interativo com pins + raio de demanda ao redor do estabelecimento |
...
| 🌱 | **Impacto** | Tela exclusiva PJ: Cadeia de Impacto Hub (CO₂, Famílias, Lixo Zerado) |
| 👤 | **Perfil** | Configurações gerais e logout |
...
Gases Evitados Totais (Brasil)   ... ton
Famílias Beneficiadas            ... k
Lixo Zerado                      ... k kgs
```
...
| Ícone | Aba | Comportamento |
|-------|-----|---------------|
| 🏠 | **Início** | Feed com destaque para doações disponíveis |
| 🗺️ | **Mapa** | Mapa com pins + heatmap de hotspots de doações por bairro |
...
| 🌱 | **Impacto** | Mesma tela do PJ, com foco em doações e famílias atendidas |
| 👤 | **Perfil** | Configurações gerais e logout |
...
| OCR on-device | google_mlkit_text_recognition | [🏪 Comerciante B2B] [👤 PF Doador] | Validação de validade (Edge AI, sem servidor) |
| GPS | geolocator | [Todos] | Captura lat/lng, permissões nativas |
...
| Tipografia | google_fonts | [Todos] | Montserrat + Nunito sem CDN externo |
...
| Take Rate | Comissão da plataforma sobre transações B2C (40% do netValue). Isento (0%) para transações B2G |
...
| OCR | Reconhecimento Óptico de Caracteres — leitura de datas de validade on-device via Google ML Kit |
| Edge AI | IA processada no dispositivo do usuário, sem envio ao servidor. Usado para o OCR de validade |
...
| B2G | Business to Government/Social — doações para ONGs. Take rate = 0% |
